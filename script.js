...
const targetWord = words[Math.floor(Math.random() * fiveLettersWords.length)];
console.log("Secret word (for testing):", targetWord);
...
submitButton.addEventListener('click', () => {
    ...
    if(guess.length !== rows) {
        alert(`Please enter a ${rows}-letter word`);
    }
    ...
    const start = currentRow * columns;
    const rowCells = [...document.querySelectorAll('.grid')].slice(start, start + columns);
      
    for (let i = 0; i < columns; i++) {
    rowCells[i].textContent = guess[i];

    if (guess[i] === targetWord[i]) {
        rowCells[i].style.backgroundColor = 'green'; // correct position
        } else if (targetWord.includes(guess[i])) {
        rowCells[i].style.backgroundColor = 'gold'; // wrong position
        } else {
        rowCells[i].style.backgroundColor = 'lightgray'; // not in word
        }
    }    
